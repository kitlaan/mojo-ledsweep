...
xil_arch ?= 32
xil_env ?= . $(isedir)/settings$(xil_arch).sh
flashsize ?= 8192
...
# some common junk
junk += *.xrpt
junk += par_usage_statistics.html usage_statistics_webtalk.html webtalk.log
junk += _xmsgs
...
$(project).bit: $(project)_par.ncd
	$(xil_env); \
	bitgen $(intstyle) -g Binary:Yes -g Compress -g DriveDone:No -g StartupClk:Cclk -w $(project)_par.ncd $(project).bit
junk += $(project).bgn $(project)_bitgen.xwbt $(project).drc $(project)_bd.bmm
junk += $(project).bit $(project).bin
...
junk += netlist.lst xst $(project).ngc $(project).ngr
